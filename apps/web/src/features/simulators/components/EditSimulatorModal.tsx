...
function createFormData(
  ...
  return {
    ...
    capacity:
      String(simulator.capacity ?? 1),

    instructor:
      simulator.instructor ?? "",

    hoursUsed:
      String(simulator.hoursUsed ?? 0),
...
export function EditSimulatorModal({
  isOpen,
  simulator,
  onClose,
  onSave,
  ...
  const [form, setForm] =
    useState<SimulatorFormData | null>(
      null,
    );

  useEffect(() => {
    if (isOpen && simulator) {
      setForm(createFormData(simulator));
    }
  }, [isOpen, simulator]);

  if (!isOpen || !simulator || !form) {
    return null;
  }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="edit-simulator-modal-title"
      onMouseDown={(event) => {
        if (
          event.target ===
          event.currentTarget
      ...
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-[#E4E7EC] bg-white shadow-[0_20px_50px_rgba(16,24,40,0.18)]">
        ...
        <header className="flex shrink-0 items-center justify-between border-b border-[#E4E7EC] px-5 py-4">
          ...
          <button
            ...
            aria-label="Close edit simulator modal"
          >
            <X className="h-4 w-4" />
          </button>
        </header>
